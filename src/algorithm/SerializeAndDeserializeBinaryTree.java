...
/**
 * layer traversal另一个例子。
 * 我的思路：这里不允许拓展数据结构，所以用了有extra space的BFS。
 * 只有非null的node才能有孩子，也才能影响数组的位移，
 * 并且它的所有孩子不管是否为null，都必须在数组里占位。
 * 任何null节点都不会对数组位移产生任何影响。
 * 因为每层有可能填不满，所以offset不能用数学方法来算，
 * 只能在上一层每遇到一个非空节点，才会严格地将下一层offset移动两个（两个孩子）
 * @author jasmineliu
 *
 */
public class SerializeAndDeserializeBinaryTree {
    ...
    public static String serialize(TreeNode root) {
        ...
        StringBuilder nextSb = new StringBuilder(root.val + ",");
        // Layer Traversal
    ...
    public static TreeNode deserialize(String data) { 
        ...
        if (data.length() < 3 || vals.length == 0 || vals[0].equals("null")) { return null; }        
        ArrayList<TreeNode> curLayer = new ArrayList<TreeNode>(), nextLayer = new ArrayList<TreeNode>();
        int i, j, offset = 1;        
        TreeNode root = new TreeNode(Integer.parseInt(vals[0])), left, right, node;
        curLayer.add(root);
        for (offset = 1; offset < vals.length; curLayer = nextLayer, nextLayer = new ArrayList<TreeNode>() ) {
        	for (i = 0, j = curLayer.size(); i < j && offset < vals.length; i++) {
        		node = curLayer.get(i);
        		if (node != null) { 
        			left = vals[offset].equals("null") ? null : new TreeNode(Integer.parseInt(vals[offset]));
        			right = vals[offset + 1].equals("null") ? null : new TreeNode(Integer.parseInt(vals[offset + 1]));  
        			nextLayer.add(left);
        			nextLayer.add(right);
        			node.left = left;
        			node.right = right;
