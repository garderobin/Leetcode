...
//		System.out.println("[0," + low + ") = " + countByCeil(size, mapping, low, 0, lowLen-1, true));
//		System.out.println("[0," + high + "] = " + countByCeil(size, mapping, high, 0, highLen-1, false));
	}
	public static int strobogrammaticInRange(String low, String high) {
		...
		// f[i][0]: the strobogrammatic size for length i which does not allow 0 as border. f[i][1] does not allow 0 borders.		
...
//			size[i][0] = size[i-2][1] * 4; // for borders type.
...
//		int nonStrict = (expectEnd < endDigit || (expectEnd == 1 && endDigit == 0))? base : base + size[len-2][1];
...
//			return  base + countByCeil(size, mapping, ceil, startIndex+1, endIndex-1, false);
			nonStrict = base + countByCeil(count, mapping, ceil, startIndex+1, endIndex-1, false, true);
		} else if (expectEnd >=0 ) {
...
//			if (newEnding > 0) {
//			return base + countByCeil(size, mapping, ceil.substring(startIndex, endIndex-1) + newEnding, 0, len-2, false);
...
//	private int countByCeil(int[][] size, int[][] mapping, String ceil, int startIndex, int endIndex, boolean strict) {
//		if (startIndex < endIndex) { return 0; }
//		if (ceil.charAt(0) == '0') { // ceil string starts with 0
//			return countByCeil(size, mapping, ceil, startIndex + 1, endIndex, strict);
//		} 
//		int startDigit = (int)(ceil.charAt(startIndex)), len = endIndex - startIndex + 1;
//		int base = mapping[startDigit][1] * size[len-1][1], expectEnd = mapping[startDigit][0];
