...
class Interval2D<T extends Comparable<T>> {
	Interval<T> intervalX;
	Interval<T> intervalY;
	T val;
	
	Interval2D(Interval<T> NE, Interval<T> SW) {
		this.intervalX = NE;
		this.intervalY = SW;
	}

	public boolean contains(T x, T y) {
		return x.compareTo(intervalX.max()) <= 0 && intervalX.min().compareTo(x) <= 0
				&& y.compareTo(intervalY.max()) <= 0 && intervalY.min().compareTo(y) <= 0;
	}
}
